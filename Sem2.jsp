...
<%    String seat = request.getParameter("t1");
      int phy= Integer.parseInt(request.getParameter("t2"));
      int chm = Integer.parseInt(request.getParameter("t3"));
      int mth= Integer.parseInt(request.getParameter("t4"));
      int cs=Integer.parseInt(request.getParameter("t5"));
      int pc=Integer.parseInt(request.getParameter("t6"));
      int be=Integer.parseInt(request.getParameter("t7"));
     
                      int total=phy+chm+mth+cs+pc+be;
	      float avg=total/5;
	      String Grade;
	      if(avg>80)
	      {
		...
		Grade="C";
      	...
      	String str="Insert into sem2 values('"+seat+"',"+phy+","+chm+","+mth+","+cs+","+pc+","+be+","+total+","+avg+",'"+Grade+"');";
 ...
 try{
	...
	ResultSet rs=dis.executeQuery();
        out.println("<table>");                                 
	out.println("<tr><th>ID</th><th>PHY</th><th>CHM</th><th>MATH</th><th>CS</th><th>PIC</th><th>BEL</th><th>TOTAL</th><th>AVERAGE</th><th>GRADE</th></tr>"   +" ");
	while(rs.next())
	 {
...
Grade=rs.getString(10);
             	...
             	out.println("<tr><td>"+seat+"</td><td>"+phy+"</td><td>"+chm+"</td><td>"+mth+"</td><td>"+cs+"</td><td>"+pc+"</td><td>"+be+"</td><td>"+total+"</td><td>"+avg+"</td><td>"+Grade+"</td></tr>");
	  }
    }
 catch(Exception e)
 {out.println(e);}
